/* ------------------------------------------------------------------
   Writer's Notepad — renderer logic (Electron)
   Rich-text editor (contenteditable) with tabs. Menus are native; the
   main process sends 'command' messages that land in doCommand().
   File IO and the session file go through window.native (preload).
------------------------------------------------------------------- */
...
/* ---------------- Platform adapter (Electron flavor) ---------------- */

const platform = {
  // -> { name, path, content } | null (cancelled)
  openFile: () => native.openFile(),

  saveFile: (path, content) => native.saveFile(path, content),

  // -> { name, path } | null (cancelled). Dialog offers .txt and .html.
  pickSaveFile: (suggestedName, preferHtml) =>
    native.pickSaveFile(suggestedName, preferHtml),
...
// Keep the native menu's radio groups (theme/typeface/alignment) in sync
function syncMenuState() {
  native.syncState({ font: state.font, align: state.align, theme: state.theme });
}

/* ---------------- Tabs ---------------- */

function createTab({ title = null, fileName = null, filePath = null, content = '', savedContent = '' } = {}) {
  const id = tabIdCounter++;
  const tab = {
    id,
    fileName,                                        // basename of the backing file, if any
    path: filePath,                                  // absolute path on disk, if any
...
function updateTabChrome(tab) {
  ...
  tab.el.title = tab.path || tab.title;
...
async function cmdSave() {
  ...
  if (!tab) return;
  if (tab.path) {
    await platform.saveFile(tab.path, serialize(tab));
    tab.savedHtml = tab.editor.innerHTML;
    updateTabChrome(tab);
    scheduleSessionSave();
  } else {
...
async function cmdSaveAs() {
  ...
  tab.path = result.path;
  tab.format = formatFor(result.name);
  await platform.saveFile(result.path, serialize(tab));
  tab.savedHtml = tab.editor.innerHTML;
  updateTabChrome(tab);
  scheduleSessionSave();
}
...
const editCommands = {
  ...
  paste: () =>
    withEditorFocus(async () => {
      ...
      } catch { /* covered by the native Edit menu role */ }
    }),
...
async function restoreSession() {
  ...
  for (const t of data.tabs) {
    createTab({
      title: t.title,
      fileName: t.fileName,
      filePath: t.path || null,   // real paths survive relaunch — Save keeps working
      content: t.content || '',
...
/* ---------------- Command dispatch (fed by the native menu) ---------------- */
...
native.onCommand((name, arg) => doCommand(name, arg));

/* ---------------- Keyboard: find/goto bars + tab cycling ---------------- */
/* Everything else (⌘S, ⌘B, …) arrives via native menu accelerators. */
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.ctrlKey && e.key === 'Tab') {
    e.preventDefault();
    doCommand(e.shiftKey ? 'prevTab' : 'nextTab');
  }
});
